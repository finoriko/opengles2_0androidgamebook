package com.lbo.book.openglbasicblendtex;
...
public class MainGLRenderer implements Renderer {
    ...
    TexMulti[] mTexMulti = new TexMulti[36];
    // 생성자
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        ...
        for(int i =0; i<6; i++) {
            for(int j=0; j< 6; j++) {
                mTexMulti[countIndex] = new TexMulti(mProgramImage);
                mTexMulti[countIndex].setBitmap(imageHandle, 150, 200);
                mTexMulti[countIndex].setPosX(-400 + (j* 150));
                mTexMulti[countIndex].setPosY(700-(i*200));
                countIndex++;
            }
        }
    }
    ...
    int[] sourceFactors = {GLES20.GL_ZERO, GLES20.GL_ONE, GLES20.GL_DST_COLOR, GLES20.GL_ONE_MINUS_DST_COLOR, GLES20.GL_SRC_ALPHA, GLES20.GL_ONE_MINUS_SRC_ALPHA};
    int[] destinationFactors = {GLES20.GL_ZERO, GLES20.GL_ONE, GLES20.GL_SRC_COLOR, GLES20.GL_ONE_MINUS_SRC_COLOR,GLES20.GL_SRC_ALPHA, GLES20.GL_ONE_MINUS_SRC_ALPHA};
    ...
    public void onDrawFrame(GL10 unused) { long now = System.currentTimeMillis();
        ...
        for(int i= 0; i<6; i++) {
            for(int j=0; j<6; j++) {
                GLES20.glBlendFunc(sourceFactors[j], destinationFactors[i]);
                mTexMulti[countIndex].draw(mMtrxProjectionAndView);
                countIndex++;
            }
        }
