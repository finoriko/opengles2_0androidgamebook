...
public class MainGLRenderer implements Renderer {
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        ...
        Bitmap image = BitmapFactory.decodeResource(mContext.getResources(),
                mContext.getResources().getIdentifier("drawable/b_spearman1", null,
                        mContext.getPackageName()));
        int imageHandle = getImageHandle(image);
        for(int i=0; i< 3; i++) {
            mTex[i] = new Tex(mProgramImage);
            mTex[i].setBitmap(imageHandle, 180, 300);
        }
    }
    // 쉐이더 이미지
    ...
    public void onDrawFrame(GL10 unused) { long now = System.currentTimeMillis();
        ...
        mTex[0].draw(mMtrxProjectionAndView);

        // 우상단의 화면

        GLES20.glViewport((int)(mDeviceWidth/2), (int) (mDeviceHeight/2), (int)(mDeviceWidth/2), (int) (mDeviceHeight/2));
        Matrix.orthoM(mMtrxProjection, 0, -300, 300, -500, 500, 0, 50);
        Matrix.multiplyMM(mMtrxProjectionAndView, 0, mMtrxProjection, 0, mMtrxView, 0);
        mTex[1].draw(mMtrxProjectionAndView);

    // 좌하단의 화면
        GLES20.glViewport(0, 0, (int) 400, (int) 500);
        Matrix.orthoM(mMtrxProjection, 0, -500, 500, -800, 800, 0, 50);
        Matrix.multiplyMM(mMtrxProjectionAndView, 0, mMtrxProjection, 0, mMtrxView, 0);
        mTex[2].draw(mMtrxProjectionAndView);
