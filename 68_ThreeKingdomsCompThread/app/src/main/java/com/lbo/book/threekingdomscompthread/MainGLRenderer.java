...
public class MainGLRenderer implements Renderer {
    ...
    // 금액관리
    private static NumberPanel mMoney;
    private static Panel[] mNumMoney = new Panel[5];

    // 팝업관리
    private static Panel mPopupPanel;
    private static Panel[] mPopupStr = new Panel[ConstMgr.POPUP_TYPE_SIZE];
    private static Button[] mPopupBtns = new Button[ConstMgr.POPUP_BUTTON_SIZE];
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        ...
        if(mIsFirstCalled == true){
           init();
            ...
            initResource();
            mResourceLoading.setResourcePopup(mPopup, mPopupPanel, mPopupStr, mPopupBtns);
    ...
    private void initObject() {
        ...
        // 금액관리
        for(int i=0; i< 5; i++){
            mNumMoney[i] = new Panel(mProgramImage,mProgramSolidColor);
        }
        mMoney = new NumberPanel(mNumMoney);
        mMoney.setPos(50, ConstMgr.SCREEN_HEIGHT -50);
        mMoney.setNumberSize(25, 50);
        mMoney.setIsActive(true);

        // 팝업
        mPopupPanel = new Panel(mProgramImage, mProgramSolidColor);
        for(int i=0; i<ConstMgr.POPUP_TYPE_SIZE; i++){
            mPopupStr[i] = new Panel(mProgramImage, mProgramSolidColor);
        }
        for(int i=0; i< 3; i++){
            mPopupBtns[i] = new Button(mProgramImage,mProgramSolidColor, this);
        }
        mPopup = new Popup(this,mPopupPanel, mPopupStr, mPopupBtns);
    ...
    private void initMap(){
        ...
        try{
            String step = mActivity.loadFiles();
            if(step.trim().length() ==0){
                step = "1";
            }
            Map.mStep = Integer.parseInt(step);
        }catch(Exception ex){}
        ...
        if(Map.mStep == 1){
            mRyubiMap.setPos(mCity[Map.mStep -1].mPosX,mCity[Map.mStep -1].mPosY + 150 );
            mGwanuMap.setPos(mCity[Map.mStep -1].mPosX -50,mCity[Map.mStep -1].mPosY+ 150 );
            mJangbiMap.setPos(mCity[Map.mStep -1].mPosX + 50,mCity[Map.mStep -1] .mPosY+ 150 );
        }
        else {
            mRyubiMap.setPos(mCity[Map.mStep  - 2].mPosX, mCity[Map.mStep  - 2].mPosY + 150);
            mGwanuMap.setPos(mCity[Map.mStep  - 2].mPosX - 50, mCity[Map.mStep  - 2].mPosY + 150);
            mJangbiMap.setPos(mCity[Map.mStep  - 2].mPosX + 50, mCity[Map.mStep  - 2].mPosY + 150);
            mRyubiMap.moveTo(mCity[Map.mStep  - 1].mPosX, mCity[Map.mStep  - 1].mPosY + 150);
            mGwanuMap.moveTo(mCity[Map.mStep  - 1].mPosX - 50, mCity[Map.mStep  - 1].mPosY + 150);
            mJangbiMap.moveTo(mCity[Map.mStep  - 1].mPosX + 50, mCity[Map.mStep  - 1].mPosY + 150);
        }
    ...
    private void RenderMap(float[] m) {
        ...
        if(ConstMgr.POPUP_TYPE_NONE != mPopup.getType()){
            mPopup.draw(m);
        }
    ...
    private void RenderGame(float[] m) {
        ...
        if(checkEnemyForceDeadAll() == true){
            if(Map.mStep == 10)
                ConstMgr.POPUP_TYPE_MODE  =  ConstMgr.POPUP_TYPE_ALL_WIN;
            else
                ConstMgr.POPUP_TYPE_MODE = ConstMgr.POPUP_TYPE_WIN;
            mPopup.setType(ConstMgr.POPUP_TYPE_MODE);
            mPopup.setIsActive(true);
        }
        else if (checkOurForceDeadAll() == true){
            ConstMgr.POPUP_TYPE_MODE  =  ConstMgr.POPUP_TYPE_LOSE;
            mPopup.setType(ConstMgr.POPUP_TYPE_MODE);
            mPopup.setIsActive(true);
        }
    ...
    private void selectTouchMap(int x, int y){
        if(mIsDraw == false)
            return;
        if(ConstMgr.POPUP_TYPE_MODE == ConstMgr.POPUP_TYPE_EXIT_MAP){
            mPopup.isSelected(x,y);
            return;
        }
        if(mBtnExit.isSelected(x,y) == true){
            ConstMgr.POPUP_TYPE_MODE = ConstMgr.POPUP_TYPE_EXIT_MAP;
            mPopup.setType(ConstMgr.POPUP_TYPE_MODE );
            mPopup.setIsActive(true);
            //mActivity.finish();
        }
    ...
    private void selectTouchGame(int x, int y){
        ...
        if(ConstMgr.POPUP_TYPE_MODE == ConstMgr.POPUP_TYPE_EXIT_GAME){
            mPopup.isSelected(x,y);
            return;
        }
        else  if(ConstMgr.POPUP_TYPE_MODE == ConstMgr.POPUP_TYPE_CANT_ADD){
            mPopup.isSelected(x,y);
            return;
        }
        else  if(ConstMgr.POPUP_TYPE_MODE == ConstMgr.POPUP_TYPE_WIN){
            mPopup.isSelected(x,y);
            return;
        }
        else  if(ConstMgr.POPUP_TYPE_MODE == ConstMgr.POPUP_TYPE_LOSE){
            mPopup.isSelected(x,y);
            return;
        }
        else  if(ConstMgr.POPUP_TYPE_MODE == ConstMgr.POPUP_TYPE_ALL_WIN){
            mPopup.isSelected(x,y);
            return;
        }

        if (mBtnExitGame.isSelected(x, y) == true) {
            ConstMgr.POPUP_TYPE_MODE = ConstMgr.POPUP_TYPE_EXIT_GAME;
            mPopup.setType(ConstMgr.POPUP_TYPE_MODE );
            mPopup.setIsActive(true);
            //initMap();  // 맵화면으로 이동
        }
    ...
    private void addOurForce(int type, int row, int col){
        ...
        while(true) {
            tempCount++;
            if(tempCount == 100){
                ConstMgr.POPUP_TYPE_MODE = ConstMgr.POPUP_TYPE_CANT_ADD;
                mPopup.setType(ConstMgr.POPUP_TYPE_MODE);
                mPopup.setIsActive(true);
                return;
            }
            ...
            if(mOurForce[mOurForceIndexCount].getIsActive() == false) {
                ...
                // 병사들의 가격
                int[] spendMoneyArray = new int[]{ 30, 40, 40, 60, 200, 0, 500, 500, 500};
                if(mMoney.getNumber() >= spendMoneyArray[type]) {
                    ...
                    mMoney.minusNumber(spendMoneyArray[type]);
                    mOurForce[mOurForceIndexCount].setIsActiveAll(true);
                    // 가까운 적찾기
                    /*
    ...
    // 아군이 패배했는지 체크
    private boolean checkOurForceDeadAll(){
        for(int i=0; i<Map.mInfoSizeRow; i++){
            for(int j=0; j<Map.mInfoSizeCol; j++){
                if(Map.mLandForceInfo[i][j][0]== ConstMgr.KIND_OUR){
                    return false;
    ...
    public void onPopupResponse(int type, int res){
        if(type == ConstMgr.POPUP_TYPE_EXIT_MAP){
            if(res == ConstMgr.POPUP_RES_YES){
                mActivity.finish();
            }
        }
        else if(type == ConstMgr.POPUP_TYPE_EXIT_GAME){
            if(res == ConstMgr.POPUP_RES_YES){
                initMap();
            }
        }
        else if(type == ConstMgr.POPUP_TYPE_CANT_ADD){

        }
        else if(type == ConstMgr.POPUP_TYPE_WIN){
            if(Map.mStep < 10)
                Map.mStep++;
            else
                Map.mStep = 1;
            ...
            try{
                mActivity.saveFiles(String.valueOf(Map.mStep));
            }catch(Exception ex){}

            initMap();
        }
        else if(type == ConstMgr.POPUP_TYPE_LOSE){
            initMap();
        }
        else if(type == ConstMgr.POPUP_TYPE_ALL_WIN){
            initMap();
        }
    }
}
