...
public class Man extends Unit {
    ...
    public void think() {
        ...
        if(mStateShow == true) {
            if(mState != ConstMgr.STATE_NORMAL){
                if(mStateWordCount > 40){
                    mStateShow =false;
                    hideForceWord();
                }
            }
        }
        if(this.mState == ConstMgr.STATE_FIGHT) {
            if (mCount % 8 < 2) {
                this.mBitmapState = 0;
            } else if (mCount % 8 < 4) {
                this.mBitmapState = 1;
            ...
            if (mPathMoveStep == true) {
                ...
                if(mPathStep == 0 || (Map.mLandForceInfo[mPath[mPathStep][ConstMgr.ROW]][mPath[mPathStep][ConstMgr.COL]][0] != ConstMgr.KIND_OUR &&
                    ...
                    mPathMoveStep = false;
                        ...
                        Map.mLandForceInfo[mPath[mPathStep][ConstMgr.ROW]][mPath[mPathStep][ConstMgr.COL]][0] == ConstMgr.KIND_ENEMY)){
                    if(mCount % 100 != 0){
                        activeForceState(ConstMgr.STATE_REQ_MAKE_WAY);  // 비켜..
                    }
                    else {
...
// 상태를 설정함
    ...
    void draw(float[] m){
        ...
        if (this.mAngle != 0 && (this.mScaleX != 1.0f || this.mScaleY != 1.0f)) {
            ...
            Matrix.scaleM(mScaleMatrix, 0, 200, 100, 1.0f);
            ...
            Matrix.multiplyMM(mMVPMatrix, 0, m, 0, mTranslationMatrix, 0);// Matrix.multiplyMM(mMVPMatrix2, 0, mMVPMatrix, 0, mScaleMatrix, 0);
...
// 최단거리 알고리즘 (좌표를 가진 2차원배열을 반환함)
                ...
                if (calcPosBlockRow >= 0 && calcPosBlockRow < Map.mInfoSizeRow && calcPosBlockCol >= 0 && calcPosBlockCol < Map.mInfoSizeCol) {
                    if(mIsActive == true) {
                        if (Map.mLandForceInfo[calcPosBlockRow][calcPosBlockCol][0] == ConstMgr.KIND_ENEMY) {
                            activeForceState(ConstMgr.STATE_FIGHT);
    ...
    public void setOrder (int order){
        ...
        if(order == ConstMgr.ORDER_ATTACK){
            activeForceState(ConstMgr.STATE_ATTACK);
            ...
            mPathMove = false;
            mPathMoveStep = false;
            activeForceState(ConstMgr.STATE_DEFENCE);
        }
    }
    public void setDead(){
