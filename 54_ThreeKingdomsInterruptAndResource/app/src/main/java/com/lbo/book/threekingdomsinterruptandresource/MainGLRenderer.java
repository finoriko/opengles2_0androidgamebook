package com.lbo.book.threekingdomsinterruptandresource;
...
public class MainGLRenderer implements Renderer {
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        ...
        if(mIsFirstCalled == true){
           init();
            ...
            initResource();
            mResourceLoading.setResourceIntro();
            mResourceLoading.setResourceMap();
    ...
    private void selectTouchGame(int x, int y){
        ...
        if(mButtons[0].isSelected(x,y)){
            mOurForce[0].activeForceState(ConstMgr.STATE_ATTACK);
            mEnemyForce[0].activeForceState(ConstMgr.STATE_ATTACK);
        }
        else if(mButtons[1].isSelected(x,y)){
            mOurForce[1].activeForceState(ConstMgr.STATE_DEAD);
            mEnemyForce[1].activeForceState(ConstMgr.STATE_DEAD);
        }
        else if(mButtons[2].isSelected(x,y)){
            mOurForce[2].activeForceState(ConstMgr.STATE_FIGHT);
            mEnemyForce[2].activeForceState(ConstMgr.STATE_FIGHT);
        }
        else if(mButtons[3].isSelected(x,y)){
            mOurForce[3].activeForceState(ConstMgr.STATE_MOVE);
            mEnemyForce[3].activeForceState(ConstMgr.STATE_MOVE);
        }
        */
        /*
        if(mMan.isSelected(x,y)== true){
