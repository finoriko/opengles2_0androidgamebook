...
public class ResourceLoading {
    ...
    // 팝업 이미지핸들
    private static int[] mHandlePopupStr = new int[ConstMgr.POPUP_TYPE_SIZE];
    private static float[] mHandlePopupStrLength = new float[ConstMgr.POPUP_TYPE_SIZE];
    private static String[] mPopupStr = { " ",
            "전투 중 나가시겠습니까?",
            "게임을 종료하시겠습니까",
            "더 이상 유닛을 추가할 수 없습니다.",
            "승리하였습니다.",
            "패배했습니다.",
            "통일을 이룩했습니다."} ;
    private static int[] mHandlePopupBtns = new int[3];
    private static int mHandleRect;
    ...
    // 금액관리
    private static int[] mHandleMoney = new int[10];
...
// 버튼 (검병)
        ...
        // 금액관리
        Bitmap[] bmpNumber = new Bitmap[10];
        for (int i = 0; i < 10; i++) {
            bmpNumber[i] = BitmapFactory.decodeResource(mContext.getResources(), mContext.getResources().getIdentifier("drawable/num" + i, null, mContext.getPackageName()));
        }
        mHandleMoney = getImageHandle(bmpNumber);
    }
...
// 팝업창 리소스 설정
    ...
    public void setResourcePopup(Popup popup, Panel popupPanel, Panel[] popupStr, Button[] btns){
        // 팝업배경
        Bitmap bmpRect = BitmapFactory.decodeResource(mContext.getResources(), mContext.getResources().getIdentifier("drawable/bg_popbase", null, mContext.getPackageName()));
        mHandleRect = getImageHandle(bmpRect);
        bmpRect.recycle();

        // 팝업문구설정
        int fontHeight = 48;
        Bitmap[] bmpPopupStr = new Bitmap[ConstMgr.POPUP_TYPE_SIZE];
        for (int i = 0; i < ConstMgr.POPUP_TYPE_SIZE; i++) {
            bmpPopupStr[i] = Bitmap.createBitmap((int) (fontHeight * mPopupStr[i].length()), (int) (fontHeight), Bitmap.Config.ARGB_8888);
            mHandlePopupStrLength[i] = mHangulBitmap.GetBitmap(bmpPopupStr[i], mPopupStr[i], fontHeight, Color.BLACK, -1, mScale);
        }
        mHandlePopupStr = getImageHandle(bmpPopupStr);

        for (int i = 0; i < ConstMgr.POPUP_TYPE_SIZE; i++) {
            bmpPopupStr[i].recycle();
        }

        // 팝업 버튼설정
        Bitmap[] bmpBtns = new Bitmap[3];
        bmpBtns[0] = BitmapFactory.decodeResource(mContext.getResources(), mContext.getResources().getIdentifier("drawable/button_yes", null, mContext.getPackageName()));
        bmpBtns[1] = BitmapFactory.decodeResource(mContext.getResources(), mContext.getResources().getIdentifier("drawable/button_no", null, mContext.getPackageName()));
        bmpBtns[2] = BitmapFactory.decodeResource(mContext.getResources(), mContext.getResources().getIdentifier("drawable/button_confirm", null, mContext.getPackageName()));
        mHandlePopupBtns = getImageHandle(bmpBtns);
        for (int i = 0; i < ConstMgr.POPUP_BUTTON_SIZE; i++){
            bmpBtns[i].recycle();
        }
        btns[0].setBitmap(mHandlePopupBtns[0], 200, 100);
        btns[1].setBitmap(mHandlePopupBtns[1], 200, 100);
        btns[2].setBitmap(mHandlePopupBtns[2], 200,100);
        popupPanel.setBitmap(mHandleRect, 1000, 600);
        for(int i=0; i< ConstMgr.POPUP_TYPE_SIZE; i++) {
            popupStr[i].setBitmap(mHandlePopupStr[i], 800, 100);
        }
    }
